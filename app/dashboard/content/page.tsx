            ...
            <div className="grid gap-8">
                ...
                <section className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="md:col-span-2">
                            <div className="mb-2 flex items-center justify-between">
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Notícias</label>
                                <button
                                    type="button"
                                    onClick={() => addListItem("newsSection", {
                                        category: emptyLocalized(), date: "", author: "Equipa ZebraTravel", image: "",
                                        layout: "top", title: emptyLocalized(), description: emptyLocalized()
                                    })}
                                    className="rounded-lg bg-green-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-green-700"
                                >
                                    + Adicionar Notícia
                            ...
                            <div className="grid gap-4">
                                {content.home.newsSection.items?.map((item: any, idx: number) => (
                                    <div key={idx} className="grid gap-3 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                                        <div className="flex items-center justify-between">
                                            <span className="text-sm font-semibold text-gray-500 dark:text-gray-400">Notícia #{idx + 1}</span>
                                            <button
                                                type="button"
                                                onClick={() => removeListItem("newsSection", idx)}
                                                className="rounded-lg bg-red-600 px-3 py-1 text-xs font-semibold text-white hover:bg-red-700"
                                            >
                                                Remover
                                            </button>
                                        ...
                                        <ImagePicker
                                            label="Imagem"
                                            value={item.image ?? ""}
                                            onChange={(url) => updateListItemField("newsSection", idx, "image", url, false)}
                                        />
                                        <input
                                            type="text"
                                            placeholder="Título"
                                            value={item.title?.[locale] ?? ""}
                                            onChange={(e) => updateListItemField("newsSection", idx, "title", e.target.value, true)}
                                            className="w-full rounded-lg border-gray-300 bg-gray-50 p-2 text-sm font-semibold dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                        />
                                        <textarea
                                            placeholder="Descrição"
                                            value={item.description?.[locale] ?? ""}
                                            onChange={(e) => updateListItemField("newsSection", idx, "description", e.target.value, true)}
                                            rows={2}
                                            className="w-full rounded-lg border-gray-300 bg-gray-50 p-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                        />
                                        <div className="grid gap-2 md:grid-cols-2">
                                            <input
                                                type="text"
                                                placeholder="Categoria (ex: Excursões)"
                                                value={item.category?.[locale] ?? ""}
                                                onChange={(e) => updateListItemField("newsSection", idx, "category", e.target.value, true)}
                                                className="w-full rounded-lg border-gray-300 bg-gray-50 p-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                            />
                                            <input
                                                type="text"
                                                placeholder="Autor"
                                                value={item.author ?? ""}
                                                onChange={(e) => updateListItemField("newsSection", idx, "author", e.target.value, false)}
                                                className="w-full rounded-lg border-gray-300 bg-gray-50 p-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                            />
                                            <input
                                                type="text"
                                                placeholder="Data (ex: 20 Março 2026)"
                                                value={item.date ?? ""}
                                                onChange={(e) => updateListItemField("newsSection", idx, "date", e.target.value, false)}
                                                className="w-full rounded-lg border-gray-300 bg-gray-50 p-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                            />
                                            <div>
                                                <label className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">Layout no grid</label>
                                                <select
                                                    value={item.layout ?? "top"}
                                                    onChange={(e) => updateListItemField("newsSection", idx, "layout", e.target.value, false)}
                                                    className="w-full rounded-lg border-gray-300 bg-gray-50 p-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                                >
                                                    <option value="top">Imagem em cima</option>
                                                    <option value="bottom">Imagem em baixo</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
